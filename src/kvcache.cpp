...
namespace KVCache
{
    ...
    Digest KVCache::make_digest(std::string_view key)
    {
        ...
        if (EVP_DigestUpdate(mdctx, key.data(), key.length()) != 1) {
            EVP_MD_CTX_free(mdctx);
        ...
        if (EVP_DigestFinal_ex(mdctx, digest_buf, &digest_len) != 1) {
            EVP_MD_CTX_free(mdctx);
        ...
        // Make sure the digest size is correct
        std::copy(digest_buf, digest_buf + kDigestLength, digest.begin());
    ...
    void KVCache::flush_mslab_to_dslab(Slab *mslab, Slab *dslab)
    {
        ...
        // flush_mslab_to_dslab does not hold reader_mutex_,
        // so mslab->nr_alloc may be changed after we load it.
        // causing underestimated of dslab's nr_alloc.
        //
        // So, I fix it in modify_index_sid.
    ...
    {
        auto nr_used = 0;
        mslab->for_each_slot([this, mslab, sid, &nr_used](Slot *slot, int slot_id)
                             {
                auto index_entry = get_index_entry(slot->key());
                if (!index_entry)
                {
